...
public class StereoCamera_OLD: MonoBehaviour {
    ...
    Vector3 webVRPosition = new Vector3(0, 1.242f, 0.827f);
    float VRDistance = 0;
    ...
    void Update()
    {
        ...
        pos.z *= -1;
        //pos.z -= VRDistance;//testing distance in VR
        myTransform.position = pos;
    }

    public void CallVRCode(bool switchToVR)
    {
        if (switchToVR)
        {
            changeMode("vr");
    ...
    {
      webVRPosition.y = 1.242f + val;//adjusting for proper VR height
    }

    void position_z(float val)
    {
      webVRPosition.z = val + 1f;//adjusting for proper VR depth
    }
    ...
    void changeMode(string mode)
    {
        switch (mode)
        {
            ...
            case "vr":
               ...
               // VRDistance = 1;//to compensate for the positional tracker
                break;
        }
    }
    ...
    static Matrix4x4 PerspectiveOffCenter(float left, float right, float bottom, float top, float near, float far)
    {
        ...
        GameObject.Find("CameraMain").GetComponent<Camera>().GetComponent<PlayerControl>().UsingVR();
